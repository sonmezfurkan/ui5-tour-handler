    ...
    return BaseController.extend("ui5.my.products.controller.Worklist", {
        ...
        onUpdateFinished : function (oEvent) {
            ...
            if (iTotalItems && oTable.getBinding("items").isLengthFinal()) {
                ...
                this.getView().getModel("worklistView").setProperty("/countAll", iTotalItems);
            } else {
        ...
        /**
         * Event handler when an icon tab filter is selected
        ...
        onSelectFilter: function(oEvent) {
            ...
            switch (sSelectedKey) {
                ...
                case "low":
                    ...
                    break;
                case "unavailable":
                    aFilter.push(new Filter("UnitsInStock", FilterOperator.LE, 0));
                    break;
                default:
                    break;
            }
        ...
        /**
         * Event handler for exporting table items
         * @public
         */
        onExport: function() {
            var oTable = this.getView().byId("table");
            var aColumns = this._getColumnConfig();
            var oSettings = {
                workbook: {
                    columns: aColumns,
        ...
        /**
         * Helper method to set counts for the filters
         * @private
         */
        _setIconFilterCounts: function() {
            ...
            oModel.read("/Products/$count", {
                ...
                success: function(iCount) {
                    this.getView().getModel("worklistView").setProperty("/countAvailable", iCount);
                }.bind(this)
            });             
            ...
            oModel.read("/Products/$count", {
                ...
                success: function(iCount) {
                    this.getView().getModel("worklistView").setProperty("/countLowOnStock", iCount);
                }.bind(this)
            });             

            oModel.read("/Products/$count", {
                filters: [new Filter("UnitsInStock", FilterOperator.LE, 0)],
                success: function(iCount) {
                    this.getView().getModel("worklistView").setProperty("/countUnavailable", iCount);
                }.bind(this)
            });
        },

        /**
         * Creates column configuration for table export
         * @private
         */
